...
- GitHub/GitLab targets plus user-chosen runner, image/container, and tags.
...
Discovery preserves one bounded query unit per confirmed capability and technology choice (falling back to confirmed goals only when both lists are empty), rank-round-robins repository results before deduplication/global limits, and records the complete query set and request count. If bounded search results cannot cover every query unit within the repository inspection limit, it fails before content inspection; an insufficient request budget fails before network access rather than omitting a confirmed choice. It resolves the default branch to a full 40-hex commit, reads the pinned Git tree and blobs, and accepts regular UTF-8 `SKILL.md` plus a root license text. It never checks out or executes upstream code.
...
- relevance compares only confirmed goals, architecture, choices, capabilities, constraints, and acceptance;
