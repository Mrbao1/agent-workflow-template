...
- Use only the user/host-selected `agent_control.default_model`; record ledger/envelope `fork_turns=0` and invoke the host spawn with `fork_turns="none"`. The sealed payload is the complete child context; never duplicate parent-chat turns. One same-model capacity retry is allowed; then stop.
...
- Record missed polling as supervisor debt. Interrupt only for deadline or a proven unchanged progress gap beyond the stall timeout. Close a platform-lost child with `finish --lost` only after three consecutive missing platform observations or a provider-verifiable decision receipt bound to the exact child, each against a fresh absence-proving snapshot; local/current-chat text alone is advisory.
...
- Trust the ledger only through its append hash chain; a legacy chainless ledger upgrades once on save, and any later break fails closed. `init --archive-existing` refuses while members are active unless `--force --force-reason <why> --source <provider-bound-source> --human-decision-receipt <path>` binds a provider-verifiable human decision.
