...
- `context.estimated_turn_overhead_tokens` (defaults `fast=2000`, `standard=3000`, `release=4000`): the estimated **per-turn host overhead** — host system-prompt replay plus turn cost. It is used for capacity invariants, not multiplied by the number of state transitions.
- `context.transition_token_increment` (defaults `fast=200`, `standard=400`, `release=800`): bounded control-plane growth per canonical TASK/CONTEXT transition. A transition inside an existing host turn does not imply another system-prompt replay, so charging the full per-turn overhead here would fabricate growth and make valid fast lifecycles deterministically hit the hard watermark. The deprecated `automatic_transition_token_increment` key remains an accepted alias with its exact legacy arithmetic until migration.
...
- **Keep measurement semantics separate**: cumulative `usage_receipts` update TASK's cumulative cost, but cannot replace an active-window estimate. The latest cumulative delta is never charged at a state transition because the same receipt pair could otherwise be replayed repeatedly. A real host turn is charged once through `contextctl account-turn --turn-id <stable-id>`; after task acceptance that checkpoint preserves a validated ordinary or installer-migration terminal origin so charging cannot erase completion. Only a future receipt whose declared semantics measure the current active window may replace that estimate.
...
Canonical transitions advance the active-window estimate monotonically by the configured transition increment. Real host/model turns independently add the per-turn overhead once under a caller-stable ID. A lower active estimate is valid only after a deliberate compaction handshake — `contextctl sync --host-compaction` with a verified host receipt — and the checkpoint reason and summary must disclose the reset; lowering an estimate by re-asserting a smaller number is not a compaction. `capsule_reduction_tokens` is a theoretical serialized handoff reduction, while `tokens_removed` is zero unless the signed host receipt proves the actual reduction. Compaction resets only the **active-window** gate: the cumulative TASK/provider cost account is untouched by it and still blocks at its own budget thresholds.
