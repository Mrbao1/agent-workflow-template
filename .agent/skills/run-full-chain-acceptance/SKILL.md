...
1. At node 4, use either an optional registered compatibility adapter (Web/Docker, API, CLI, iOS simulator, or workflow-governance) or the generic `blueprintacceptance.py` path selected by an `agent-template-route/v3` adaptive binding. The generic path seals the exact source inventory once, privately rematerializes a fresh candidate and HOME/TMP/XDG runtime for every confirmed argv command and every replay, and rechecks every bound file byte, digest, path kind and mode after each command; a writable zero-exit row is not execution proof. `evidence`/`manual` IDs require exact evidence coverage in a current `agent-blueprint-integrator-evidence/v1` receipt. Manual IDs additionally require a repository-external, OS-protected host/provider-verifiable human approval over the exact blueprint/candidate/Skill-lock/preflight/criterion/evidence action; current-chat `user:` text alone is rejected. Its preflight probes command availability and binds candidate/environment/authority, verified Skill lock, blueprint and runner; the v2 final receipt replays those bindings and expires within 24 hours. The node-7 live receipt's path/SHA-256/bytes must be the selected integrator's marker-bound result evidence—an asserted integrator ID alone is never authority. It does not infer a stack. Read [runner-adapters.md](references/runner-adapters.md). A declared-only legacy adapter fails at node 4.
...
7. Always stop and clean local services in success and failure paths. Docker acceptance first proves the unique project labels and exact candidate image tag are unused. It then requires the exact non-empty application-service set confirmed by the user, including the health target, to run the canonical image ID loaded from the newly built OCI archive; an unrelated prebuilt Compose service or separately asserted digest is not candidate execution. Only after the clean namespace claim may cleanup remove the project's containers, networks, volumes and exact candidate image tag, then prove all four residual inventories are zero. The generic runner and `testrun.py` use lightweight PID/stable-start ancestry polling; Linux additionally enables child subreaping, while Darwin performs only bounded per-launch environment-identity scans at final observation and cleanup. Joined children may finish normally, while a live `setsid`/double-fork launch identity is terminated and makes the Case non-success; unavailable observation or cleanup is fail-closed. This is still bounded same-UID lifecycle cleanup, not cgroup/container confinement, and it does not provide OS filesystem or network confinement. Commands run only from the descriptor-bound private candidate; if a required dependency is absent from that bounded tree, execution fails closed instead of falling back to writable project source.
...
8. R5 read-only integrator owns the final candidate evidence. Every implemented adapter exposes the same `preflight`, `run`, and `verify` boundary. Workflow, API, CLI and iOS compatibility adapters wrap an exact candidate-bound `agent-test-receipt/v3`; Web/Docker wraps a runtime receipt carrying the same canonical candidate fingerprint, and their node-completion `verify` stays read-only. The stack-neutral adaptive blueprint path has no provider-owned runner receipt, so its `verify` deliberately replays every confirmed bounded argv command through the canonical no-shell runner rather than trusting writable result rows; include that replay in the release time budget. Neither path performs undeclared commands or cleanup.
...
Use the bounded runtime helper only when the selected adapter is Web/Docker. Before execution, the user-confirmed Blueprint must contain the exact non-empty `design.application_services` Compose service set; the helper verifies its provider-observed confirmation receipt and accepts no CLI substitute:
