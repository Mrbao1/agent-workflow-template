...
- Subsequent untagged v4.0.3 candidate runs exposed the remaining platform contracts: shard-local tests are now serial so process observers cannot attribute sibling tests, GitLab drops to a dedicated bounded unprivileged account after root-only tool staging, protected `env` interpreter symlinks are accepted only through immutable OS-owned chains, contained broken framework header symlinks do not invalidate the selected macOS runtime, Linux zombie descendants are recognized as terminated without reaping a launch identity before cleanup, release preflights suppress candidate-mutating Python caches, protected lsof discovery skips bounded untrusted PATH shadows, privileged CI Python bootstraps exclude checkout import paths and keep downloaded archives off the candidate filesystem, the adopter contract names its trusted lsof prerequisite and pins current Python 3.14 coverage, generic protected authorities accept provider-native opaque immutable/actor identities, stale local-approval guidance is removed, short-lived installer sessions permit only proven member disappearance, and pxpipe recovery uses portable Node metadata plus step-specific fail-closed diagnostics while preserving the primary failure status.
