...
class WorkflowHardeningTests(unittest.TestCase):
    ...
    def test_protected_workflow_pins_actions_and_covers_all_contexts(self):
        ...
        self.assertIn("Validate external required-verifier configuration", text)
        for name in ("AGENT_GITHUB_AUTHORITY_MODE", "AGENT_GITHUB_AUTHORITY_REPOSITORY_ID",
                     "AGENT_GITHUB_AUTHORITY_WORKFLOW_REF", "AGENT_GITHUB_REQUIRED_CHECK_NAME",
                     "AGENT_GITHUB_RULESET_RECEIPT_SHA256", "AGENT_GITHUB_VERIFIER_CONFIG_SHA256"):
            self.assertIn("vars."+name,text)
    ...
    def test_canonical_gitlab_pipeline_is_pinned_and_full(self):
        ...
        self.assertNotIn("--full --context", text)
        self.assertIn('CONTEXT: ["idle-source", "polluted-source", "installed-project"]',text)
        self.assertIn(".gitlab-host-toolchain-matrix:\n  stage: verify",text)
        self.assertIn("candidate-selftest-modern-linux-evidence",text)
        self.assertIn("candidate-selftest-minimum-macos-evidence",text)
        self.assertIn("candidate-selftest-modern-macos-evidence",text)
        self.assertIn("EXPECTED_PLATFORM: darwin",text)
        self.assertIn("EXPECTED_PY_MINOR: \"13\"",text)
        self.assertIn("--require-command python --require-command node --require-command git",text)
        self.assertIn("git --version",text)
        suite_line = next(line for line in text.splitlines() if "tests/run_all.py" in line)
        self.assertIn("env -i ", suite_line)
        self.assertIn("--fail-on-skip", suite_line)
        self.assertIn("--allow-skip .agent/skills/manage-local-runtime/scripts/self_test_docker_http.py", suite_line)
        self.assertNotIn("GITLAB_TOKEN", suite_line)
