...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="install-lifecycle-") as raw:
        ...
        if installer_module.RELEASED_MANIFEST_METADATA["agent-workflow-install/v5"]!={("4.0.0",42),("4.0.1",42),("4.0.2",42),("4.0.3",42)}:
        ...
        old_bytes=old_script.read_bytes().replace(b'manifest.get("version")!="4.0.3"',b'manifest.get("version")!="4.0.0"')
...
"""
        ...
        for field, bad_value, expected_text in (
            ("version", "4.0.4", "schema/version/migration combination is not a supported release"),
        ...
        for field, bad_value in (("version", "4.0.4"), ("migration_version", 41)):
            candidate_manifest = json.loads(baseline_manifest)
            candidate_manifest[field] = bad_value
